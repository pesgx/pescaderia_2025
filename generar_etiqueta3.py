...
def generar_etiquetas_pdf(data):
    ...
    alto_etiqueta = 8 * cm  # Ajuste de altura basado en el diseño
...
# Función para añadir el contenido de una etiqueta según el diseño
def generar_contenido_etiqueta(c, x, y, registro):
    c.drawString(x + 0.5 * cm, y, f"ZONA CAPTURA: {registro['zona']}")
    c.drawString(x + 0.5 * cm, y - 0.5 * cm, f"EXPEDIDOR: {registro['expedidor']}")
    c.drawString(x + 0.5 * cm, y - 1.5 * cm, "DENOMINACIÓN COMERCIAL:")
    c.drawString(x + 1 * cm, y - 2 * cm, registro['especie'])
    c.drawString(x + 0.5 * cm, y - 3 * cm, f"MÉTODO PRODUCCIÓN: {registro['metodo']}")
    c.drawString(x + 0.5 * cm, y - 3.5 * cm, f"PVP: {registro['pvp']}")
    c.drawString(x + 0.5 * cm, y - 4 * cm, f"MODO PRESENTACIÓN: {registro['presentacion']}")
    c.drawString(x + 0.5 * cm, y - 4.5 * cm, f"ARTE: {registro['arte']}")
    c.drawString(x + 0.5 * cm, y - 5 * cm, f"BARCO: {registro['barco']}")
    c.drawString(x + 0.5 * cm, y - 5.5 * cm, f"DESCONGELADO: {registro['descongelado']}")
    c.drawString(x + 0.5 * cm, y - 6 * cm, f"LOTE EXTERNO: {registro['lote_ext']}")
    c.drawString(x + 0.5 * cm, y - 6.5 * cm, f"NOTA INTERNA: {registro['nota_int']}")
    c.drawString(x + 0.5 * cm, y - 7 * cm, f"LOTE INTERNO: {registro['lote_int']}")
