...
class CrudArtes(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Gestión de Artes")
    ...
    def cargar_datos(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id_arte, nombre_arte, descr_arte FROM tabla_artes")
        rows = cursor.fetchall()

        # Insertar datos en el treeview
        for row in rows:
            self.tree.insert("", tk.END, values=(row['id_arte'], row['nombre_arte'], row['descr_arte']))
    ...
    def guardar(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tabla_artes (nombre_arte, descr_arte) VALUES (?, ?)", (nombre, descripcion))
        conn.commit()
        conn.close()

        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Arte guardado correctamente")

    def actualizar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione un arte para actualizar")
            return

        id_arte = self.tree.item(selected_item)['values'][0]
        nombre = self.entry_nombre.get()
        descripcion = self.entry_descripcion.get()
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabla_artes SET nombre_arte = ?, descr_arte = ? WHERE id_arte = ?", (nombre, descripcion, id_arte))
        conn.commit()
        conn.close()

        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Arte actualizado correctamente")

    def eliminar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione un arte para eliminar")
            return

        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este arte?"):
            id_arte = self.tree.item(selected_item)['values'][0]

            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tabla_artes WHERE id_arte = ?", (id_arte,))
            conn.commit()
            conn.close()

            self.limpiar_campos()
            self.cargar_datos()
            messagebox.showinfo("Éxito", "Arte eliminado correctamente")
