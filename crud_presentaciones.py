...
class CrudPresentaciones(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Gestión de Presentaciones")
    ...
    def cargar_datos(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id_presentacion, nombre_presentacion, descr_presentacion FROM tabla_presentaciones")
        rows = cursor.fetchall()
        # Insertar datos en el treeview
        for row in rows:
            self.tree.insert("", tk.END, values=(row['id_presentacion'], row['nombre_presentacion'], row['descr_presentacion']))
        conn.close()
    ...
    def guardar(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tabla_presentaciones (nombre_presentacion, descr_presentacion) VALUES (?, ?)", (nombre, descripcion))
        conn.commit()
        conn.close()
        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Presentación guardada correctamente")

    def actualizar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione una presentación para actualizar")
            return
        id_presentacion = self.tree.item(selected_item)['values'][0]
        nombre = self.entry_nombre.get()
        descripcion = self.entry_descripcion.get()
        if not nombre:
            messagebox.showerror("Error", "El nombre es obligatorio")
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabla_presentaciones SET nombre_presentacion = ?, descr_presentacion = ? WHERE id_presentacion = ?", (nombre, descripcion, id_presentacion))
        conn.commit()
        conn.close()
        self.limpiar_campos()
        self.cargar_datos()
    ...
    def eliminar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione una presentación para eliminar")
            return
        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar esta presentación?"):
            id_presentacion = self.tree.item(selected_item)['values'][0]
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tabla_presentaciones WHERE id_presentacion = ?", (id_presentacion,))
            conn.commit()
            conn.close()
            self.limpiar_campos()
            self.cargar_datos()
