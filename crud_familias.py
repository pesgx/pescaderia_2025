...
class CrudFamilias(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Gestión de Familias")
    ...
    def cargar_datos(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id_familia, nombre_familia, descr_familia FROM tabla_familias")
        rows = cursor.fetchall()

        # Insertar datos en el treeview
        for row in rows:
            self.tree.insert("", tk.END, values=(row['id_familia'], row['nombre_familia'], row['descr_familia']))
    ...
    def guardar(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tabla_familias (nombre_familia, descr_familia) VALUES (?, ?)", (nombre, descripcion))
        conn.commit()
        conn.close()
    ...
    def actualizar(self):
        ...
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione una familia para actualizar")
            return

        id_familia = self.tree.item(selected_item)['values'][0]
        nombre = self.entry_nombre.get()
        descripcion = self.entry_descripcion.get()
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabla_familias SET nombre_familia = ?, descr_familia = ? WHERE id_familia = ?", (nombre, descripcion, id_familia))
        conn.commit()
        conn.close()

        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Familia actualizada correctamente")

    def eliminar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione una familia para eliminar")
            return

        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar esta familia?"):
            id_familia = self.tree.item(selected_item)['values'][0]
            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tabla_familias WHERE id_familia = ?", (id_familia,))
            conn.commit()
            conn.close()
