...
class CrudMetodos(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Gestión de Métodos")
    ...
    def cargar_datos(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id_metodo, nombre_metodo, descr_metodo FROM tabla_metodos")
        rows = cursor.fetchall()

        # Insertar datos en el treeview
        for row in rows:
            self.tree.insert("", tk.END, values=(row['id_metodo'], row['nombre_metodo'], row['descr_metodo']))
    ...
    def guardar(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tabla_metodos (nombre_metodo, descr_metodo) VALUES (?, ?)", 
                       (nombre, descripcion))
        conn.commit()
        conn.close()
        ...
        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Método guardado correctamente")

    def actualizar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione un método para actualizar")
            return

        id_metodo = self.tree.item(selected_item)['values'][0]
        nombre = self.entry_nombre.get()
        descripcion = self.entry_descripcion.get()
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabla_metodos SET nombre_metodo = ?, descr_metodo = ? WHERE id_metodo = ?", 
                       (nombre, descripcion, id_metodo))
        conn.commit()
        conn.close()

        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Método actualizado correctamente")

    def eliminar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione un método para eliminar")
            return

        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este método?"):
            id_metodo = self.tree.item(selected_item)['values'][0]

            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tabla_metodos WHERE id_metodo = ?", (id_metodo,))
            conn.commit()
            conn.close()

            self.limpiar_campos()
            self.cargar_datos()
            messagebox.showinfo("Éxito", "Método eliminado correctamente")
