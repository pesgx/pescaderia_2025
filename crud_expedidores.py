...
class CrudExpedidores(tk.Toplevel):
    def __init__(self, master):
        super().__init__(master)
        self.title("Gestión de Expedidores")
    ...
    def cargar_datos(self):
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT id_expedidor, nombre_expedidor, descr_expedidor FROM tabla_expedidores")
        rows = cursor.fetchall()

        # Insertar datos en el treeview
        for row in rows:
            self.tree.insert("", tk.END, values=(row['id_expedidor'], row['nombre_expedidor'], row['descr_expedidor']))
    ...
    def guardar(self):
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("INSERT INTO tabla_expedidores (nombre_expedidor, descr_expedidor) VALUES (?, ?)", 
                       (nombre, descripcion))
        conn.commit()
        conn.close()
        ...
        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Expedidor guardado correctamente")

    def actualizar(self):
        selected_item = self.tree.selection()
        if not selected_item:
             
            messagebox.showerror("Error", "Por favor, seleccione un expedidor para actualizar")
            return

        id_expedidor = self.tree.item(selected_item)['values'][0]
        nombre = self.entry_nombre.get()
        descripcion = self.entry_descripcion.get()
        ...
        conn = get_db_connection()
        cursor = conn.cursor()
        cursor.execute("UPDATE tabla_expedidores SET nombre_expedidor = ?, descr_expedidor = ? WHERE id_expedidor = ?", 
                       (nombre, descripcion, id_expedidor))
        conn.commit()
        conn.close()

        self.limpiar_campos()
        self.cargar_datos()
        messagebox.showinfo("Éxito", "Expedidor actualizado correctamente")

    def eliminar(self):
        selected_item = self.tree.selection()
        if not selected_item:
            messagebox.showerror("Error", "Por favor, seleccione un expedidor para eliminar")
            return

        if messagebox.askyesno("Confirmar", "¿Está seguro de que desea eliminar este expedidor?"):
            id_expedidor = self.tree.item(selected_item)['values'][0]

            conn = get_db_connection()
            cursor = conn.cursor()
            cursor.execute("DELETE FROM tabla_expedidores WHERE id_expedidor = ?", (id_expedidor,))
            conn.commit()
            conn.close()

            self.limpiar_campos()
            self.cargar_datos()
            messagebox.showinfo("Éxito", "Expedidor eliminado correctamente")
